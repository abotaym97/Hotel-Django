from . import views
from django.urls import path
from .views import booking_settings,admin_restaurants, cancel_booking,toggle_maintenance,site_status,auto_close_setting,dashboard_occupancy,notification_detail,all_bookings,dashboard_stats, delete_booking, delete_room, get_hotels , get_rooms , bookings , available_rooms , booking_detail , register , my_bookings , profile, hotel_detail, room_types, rooms_by_hotel, update_room, zaincash_payment


urlpatterns = [
    path('hotels/' , get_hotels),
    path('rooms/' , get_rooms),
    path('bookings/' , bookings),
    path('available_rooms/',available_rooms),
    path('bookings/<int:booking_id>/', booking_detail),
    path('bookings/<int:booking_id>/zaincash/',zaincash_payment),
    path('register/', register),
    path('my-bookings/', my_bookings),
    path('profile/', profile),
    path('hotels/<int:id>/', hotel_detail),
    path('hotels/<int:hotel_id>/rooms/', rooms_by_hotel),
    path('booking-settings/', booking_settings),
    path('dashboard-stats/', dashboard_stats),
    path('all-bookings/', all_bookings),
    path('rooms/<int:pk>/', views.room_detail),
    path('rooms/<int:pk>/delete/', delete_room),
    path('rooms/<int:pk>/update/', update_room),
    path('bookings/<int:pk>/delete/', delete_booking),
    path("bookings/<int:pk>/cancel/",cancel_booking,name="cancel_booking"),
    path('room-types/', room_types),
    path("room-types/<int:pk>/", views.room_type_detail),
    path("admin/room-types/<int:pk>/", views.admin_room_type_detail),
    path('admin/bookings/', views.admin_bookings),
    path('admin/current-bookings/', views.current_bookings),
    path('room-types/<int:pk>/rooms/', views.rooms_by_type),
    path("room-types/", views.room_types),
    path("admin/room-types/", views.admin_room_types),
    path('rooms/bulk-availability/', views.bulk_room_availability),
    path('restaurants/', views.restaurants),
    path("restaurants/<int:pk>/", views.restaurant_detail),          # GET عام
    path("admin/restaurants/", views.admin_restaurants),
    path("admin/restaurants/<int:pk>/", views.admin_restaurant_detail),    # PUT DELETE أدمن
    path('nearby-places/', views.nearby_places),
    path("admin/nearby-places/", views.admin_nearby_places),
    path('nearby-places/<int:pk>/', views.nearby_place_detail),
    path('admin/nearby-places/<int:pk>/', views.admin_nearby_place_detail),
    path('services/', views.services),
    path('services/<int:pk>/', views.service_detail),
    path("admin/services/", views.admin_services),
    path('admin/services/<int:pk>/', views.admin_service_detail),
    path('galleries/', views.galleries),
    path('galleries/<int:pk>/', views.gallery_detail),
    path('admin/galleries/', views.admin_galleries),
    path('admin/galleries/<int:pk>/', views.admin_gallery_detail),
    path('gallery-images/', views.gallery_images),
    path('gallery-images/<int:pk>/', views.gallery_image_detail),
    path('reviews/', views.reviews),
    path('admin/reviews/', views.admin_reviews),
    path('reviews/<int:pk>/', views.review_detail),
    path('submit-review/', views.submit_review),
    path('staff-users/', views.staff_users),
    path('staff-users/<int:pk>/', views.staff_user_detail),
    path('groups/', views.groups),
    path('groups/<int:pk>/', views.group_detail),
    path('permissions/', views.permissions_list),
    path('logs/', views.logs),
    path('public-contact-settings/', views.public_contact_settings),
    path('contact-settings/', views.contact_settings),
    path('contact-messages/', views.submit_contact_messages),
    path('admin/contact-messages/', views.contact_messages),
    path('admin/contact-messages/<int:pk>/', views.contact_message_detail),
    path('notifications/', views.notifications),
    path('notifications/<int:pk>/', views.notification_detail),
    path('dashboard-card-settings/',views.dashboard_card_settings),
    path('dashboard-card-settings/<int:pk>/',views.dashboard_card_setting_detail),
    path("system-settings/", views.system_settings),
    path("admin/system-settings/", views.admin_system_settings),
    path("admin-profiles/", views.admin_profiles),
    path("admin-profiles/<int:pk>/", views.admin_profile_detail),
    path("customer-records/", views.customer_records),
    path('rooms/<int:room_id>/bookings/',views.room_bookings),
    path('rooms/<int:room_id>/availability/',views.room_availability),
    path("rooms/bulk-off/", views.rooms_bulk_off),
    path("bookings/<int:booking_id>/toggle-read/",views.toggle_booking_read),
    path("admin-meal-options/<int:meal_id>/", views.admin_meal_option_detail),
    path("meal-options/", views.meal_options),
    path("admin-meal-options/", views.admin_meal_options),
    path("meal-options/<int:meal_id>/toggle/",views.toggle_meal_option),
    path("currencies/", views.currencies),
    path("admin/currencies/", views.admin_currencies),
    path("admin/currencies/<int:pk>/", views.currency_detail),
    path("bookings/<int:booking_id>/details/", views.booking_detail),
    path("bookings/<int:booking_id>/notes/", views.update_booking_notes),
    path("bookings/<int:booking_id>/status/", views.update_booking_status),
    path("bookings/<int:booking_id>/pay/",views.fake_payment),
    path("bookings/<int:booking_id>/",views.update_booking_payment),
    path("bookings/mark-all-read/",views.mark_all_bookings_read),
    path("hero-slides/", views.active_hero_slides),
    path("admin-hero-slides/", views.admin_hero_slides),
    path("admin-hero-slides/<int:pk>/", views.admin_hero_slide_detail),
    path("amenities/", views.amenities),
    path("dashboard/occupancy/", dashboard_occupancy),
    path("table-settings/<str:table_name>/",views.user_table_setting,name="user-table-setting"),
    path("dashboard/settings/", auto_close_setting),
    path("site/status/", site_status),
    path("site/toggle-maintenance/", toggle_maintenance),
    path("facilities/", views.facilities),
    path("admin/facilities/", views.admin_facilities),
    path("admin/facilities/<int:pk>/", views.admin_facility_detail),
    path("policy-settings/", views.public_policy_settings),
    path("admin/policy-settings/", views.policy_settings),
    path("social-settings/", views.public_social_settings),
    path("admin/social-settings/", views.social_settings),
    path("profile/", views.profile),
    path("change-password/", views.change_password),
    path("delete-account/", views.delete_account),
]

